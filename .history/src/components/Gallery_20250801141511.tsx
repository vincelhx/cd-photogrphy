...
const Gallery: React.FC<GalleryProps> = ({ photos }) => {
  ...
  const updateActiveSlide = () => {
    ...
    console.log(`Closest slide index: ${closest}`);
  ...
  useEffect(() => {
    ...
    // Initial scroll to center item only once at mount
    scrollToIndex(Math.floor(photos.length / 2));
  
    return () => {
      container.removeEventListener('scroll', updateActiveSlide);
      window.removeEventListener('resize', updateActiveSlide);
    };
  }, [Math.floor(photos.length / 2)]); // <= empty deps means only run on mount
  //default : length/2
...
{activeIndex > 0 && (
  ...
  >
    ...
    {activeIndex < photos.length - 1 && (
      <button
        onClick={() => scrollToIndex(activeIndex + 1)}
        className="absolute z-20 top-1/2 -translate-y-1/2 right-[calc(50%-25vw)] bg-white/90 hover:bg-white rounded-full shadow px-3 py-2 text-2xl transition duration-200"
      >
        ▶
      </button>
    )}
