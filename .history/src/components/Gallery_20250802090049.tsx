...
const Gallery: React.FC<GalleryProps> = ({ photos }) => {
  ...
  return (
    <div className="relative w-full h-[80vh] flex items-center justify-center bg-black border-4 border-dashed border-red-500">
      {/* PHOTOS */}
      ...
      <div
        ...
        className={`
          ${index === activeIndex ? 'relative z-20' : 'z-10'}
          shrink-0 w-[45vw] max-w-[700px] h-[60vh]
          flex flex-col justify-between bg-white rounded-xl overflow-hidden shadow-xl snap-center transition-all duration-300
          ${index === activeIndex 
          ...
          return (
            <div
              ...
              className={`shrink-0 w-[45vw] max-w-[700px] h-[60vh] flex flex-col justify-between bg-white rounded-xl overflow-hidden shadow-xl snap-center transition-all duration-300
                 ${
                   index === activeIndex
                     ? "w-[60vw] max-w-[9000px] scale-100 opacity-100 blur-0 z-50"
              ...
              {index === activeIndex && (
                <>
                  {/* ◀ Flèche gauche : sortie de 6% vers la gauche */}
                  <button
                    onClick={shiftLeft}
                    className="absolute left-[-2.5rem] top-1/2 -translate-y-1/2 z-[999] bg-white text-black rounded-full border border-black px-5 py-3 text-[clamp(2rem,4vw,3rem)] shadow">
                    ◀
                  </button>

                  {/* ▶ Flèche droite : sortie de 6% vers la droite */}
                  <button
                    onClick={shiftRight}
                    className="absolute right-0 translate-x-[120%] top-1/2 -translate-y-1/2 z-[999] bg-white text-black rounded-full border border-black px-5 py-3 text-[clamp(2rem,4vw,3rem)] shadow">
                    ▶
                  </button>
                </>
              )}
