...
const Gallery: React.FC<GalleryProps> = ({ photos }) => {
  ...
  const scrollToIndex = (index: number) => {
    ...
    if (container && slide) {
      ...
      console.log({ scrollPosition, containerCenter, slideCenter })
      container.scrollTo({
        left: scrollPosition,
        behavior: 'smooth',
      });
  ...
  const updateActiveSlide = () => {
    ...
    Array.from(container.querySelectorAll('[data-slide]'))
.forEach((child, index) => {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      setTimeout(updateActiveSlide, 300); // ← force après scroll smooth
    }, 50);
  ...
  return  (
    <div
      ...
      className={`shrink-0 w-[45vw] max-w-[700px] h-[60vh] flex flex-col justify-between bg-white rounded-xl overflow-hidden shadow-xl snap-center transition-all duration-300 ${
        index === activeIndex
          ? 'scale-100 opacity-100 blur-0 border-green-500'
          : 'scale-95 opacity-60 blur-sm border-transparent'
      }`}
    >
      {/* Image container */}
...
{index === activeIndex - 1 && (
  <button
    ...
    className="absolute top-1/2 left-1/2 -translate-x-full -translate-y-1/2 z-50 bg-white text-black font-bold rounded-full shadow border border-black px-5 py-3 text-[clamp(2rem,5vw,4rem)]"
  >
    ◀
  </button>
)}
