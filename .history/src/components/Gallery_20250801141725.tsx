    ...
    <div className="relative w-full h-[80vh] flex items-center justify-center bg-white">
      ...
      {activeIndex > 0 && (
        <button
          ...
          className="absolute z-20 top-1/2 -translate-y-1/2 left-[calc(50%-25vw)] bg-white/90 hover:bg-white rounded-full shadow px-3 py-2 text-2xl transition duration-200"
        >
          ◀
        </button>
      )}
...
{photos.map((photo, index) => (
  <div
    key={photo.id}
    className={`relative shrink-0 w-[45vw] max-w-[700px] h-[60vh] flex flex-col justify-between bg-white rounded-xl overflow-hidden shadow-xl snap-center transition-all duration-300 ${
      index === activeIndex
  ...
  >
    ...
    {/* LEFT ARROW - shown only on left-side card */}
    {index === activeIndex - 1 && (
      <button
        onClick={() => scrollToIndex(activeIndex - 1)}
        className="absolute left-4 top-1/2 -translate-y-1/2 z-10 bg-white/80 hover:bg-white rounded-full shadow p-2 text-xl"
      >
        ◀
      </button>
    )}

    {/* RIGHT ARROW - shown only on right-side card */}
    {index === activeIndex + 1 && (
      <button
        onClick={() => scrollToIndex(activeIndex + 1)}
        className="absolute right-4 top-1/2 -translate-y-1/2 z-10 bg-white/80 hover:bg-white rounded-full shadow p-2 text-xl"
      >
        ▶
      </button>
    )}
