...
const Gallery: React.FC<GalleryProps> = ({ photos }) => {
  ...
  const galleryRef.current;
    if (!container) retupdateActiveSlide = () => {
    const container = urn;
...
{photos.map((photo, index) => (
  ...
  {index === activeIndex - 1 && (
    <button
      ...
      className="absolute left-4 top-1/2 -translate-y-1/2 z-30 bg-white text-black font-bold rounded-full shadow border border-black px-3 py-2 text-xl"
    >
      ◀
    </button>
  )}
  <button className="absolute top-1/2 left-1/2 z-50 bg-red-600 text-white p-4 rounded-full">
  Test
  </button>
  ...
  {index === activeIndex + 1 && (
    <button
      onClick={() => scrollToIndex(activeIndex + 1)}
      className="absolute right-4 top-1/2 -translate-y-1/2 z-30 bg-white text-black font-bold rounded-full shadow border border-black px-3 py-2 text-xl"
    >
      ▶
    </button>
  )}
