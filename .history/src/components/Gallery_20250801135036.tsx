...
const Gallery: React.FC<GalleryProps> = ({ photos }) => {
  ...
  return (
<div className="relative w-full h-[80vh] flex items-center justify-center overflow-hidden bg-black/5">
  <button
      ...
      >
        {photos.map((photo, index) => (
          <div
            key={photo.id}
            className={`shrink-0 w-[45vw] max-w-[700px] rounded-xl flex flex-col justify-between bg-white overflow-hidden shadow-xl snap-center transition-all duration-300 ${
              index === activeIndex
                ? 'scale-100 opacity-100 blur-0'
                : 'scale-95 opacity-60 blur-sm'
            }`}
          >
            <img src={photo.src} alt={photo.title} className="w-full h-full block" />
