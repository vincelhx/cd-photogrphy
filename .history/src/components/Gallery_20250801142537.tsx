...
className="absolute left-4 top-1/2 -translate-y-1/2 z-20 bg-black hover:bg-red rounded-full shadow p-2 text-xl"
        >
          ◀
        </button>
      )}
      ...
      {index === activeIndex + 1 && (
        <button
          onClick={() => scrollToIndex(activeIndex + 1)}
          className="absolute right-4 top-1/2 -translate-y-1/2 z+222220 bg-black text-white border border-white hover:bg-red rounded-full shadow p-2 text-xl"
          >
          ▶
        </button>
      )}
