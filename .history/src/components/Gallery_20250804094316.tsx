...
const Gallery: React.FC<GalleryProps> = ({ photos }) => {
  ...
  */
  ...
  const visibleCount = 5; // nombre d’images visibles
  const middleIndex = Math.floor(visibleCount / 2);

  const circular = [...photos, ...photos, ...photos]; // padding pour éviter les erreurs
  const start = activeIndexViz + photos.length; // pour éviter les négatifs
  const photosCircular = circular.slice(start - middleIndex, start + middleIndex + 1);
