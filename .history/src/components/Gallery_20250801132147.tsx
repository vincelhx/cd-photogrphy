...
>
  {photos.map((photo, index) => (
    <div
      key={photo.id}
      className="flex-none w-[40vw] max-w-[600px] bg-white rounded-xl shadow snap-center"
    >
      <img src={photo.src} alt={photo.title} className="w-full h-auto" />
      <p className="text-center bg-gray-100 text-gray-800 p-4">{photo.title}</p>
    </div>
  ))}
</div>
      ...
      >
        {photos.map((photo, index) => (
          <div
            key={photo.id}
            className={`flex-none w-[40vw] max-w-[600px] rounded-xl bg-white overflow-hidden shadow-xl snap-center transition-all duration-300 ${
              index === activeIndex
