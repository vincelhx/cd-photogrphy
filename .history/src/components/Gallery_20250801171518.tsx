...
const Gallery: React.FC<GalleryProps> = ({ photos }) => {
  ...
  return (
    <div className="relative w-full h-[80vh] flex items-center justify-center bg-white">
      ...
      <button
        onClick={shiftLeft}
        className="absolute left-1/16 top-1/2 -translate-y-1/2 z-50 bg-white text-black font-bold rounded-full shadow border border-black px-5 py-3 text-[clamp(2rem,4vw,3rem)]">
        ◀
      </button>
      <div
        ref={galleryRef}
      ...
      <button
        onClick={shiftRight}
        className="absolute right-1/2 top-1/2 -translate-y-1/2 z-50 bg-white text-black font-bold rounded-full shadow border border-black px-5 py-3 text-[clamp(2rem,4vw,3rem)]">
        ▶
      </button>
    </div>
  );
